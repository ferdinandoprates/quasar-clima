  ...
  <q-page class="flex column">
    <div class="col q-pa-lg">
      <q-input
        ...
        dark
        borderless
        @keyup.enter="pegarlocalizacaoBusca()"
      >
        <template v-slot:before>
          <q-icon
            @click="pegarlocalizacaoBusca"
            name="my_location"
    ...
    <template v-else>
      <div class="col column text-center text-white">
        <div class="col text-h2 text-weight-thin">Clima</div>
    ...
    <div class="footer text-grey text-subtitle2">FERDINANDO PRATES ©2022</div>
  </q-page>
</template>
  ...
  methods: {
    pegarlocalizacaoBusca() {
      if (!this.busca) {
        ...
        this.weatherData = "";
        this.$axios(
          `${this.apiurl}q=${this.busca}&appid=${this.apikey}&lang=pt_br&units=metric`
        )
          .then((resp) => {
    ...
    getWeatherByCoords() {
      this.$axios(
        `${this.apiurl}lat=${this.lat}&lon=${this.lon}&appid=${this.apikey}&lang=pt_br&units=metric`
      ).then((resp) => {
